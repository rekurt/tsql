...
use super::sql::{
    effective_max_rows, escape_sql_string_literal, extract_table_from_query, is_pageable_query,
    is_row_returning_query, parse_schema_tree_identifier, resolve_ssl_mode, SchemaTableContext,
    SchemaTreeSelection, META_QUERY_COLUMN_TYPES, META_QUERY_DATABASES, META_QUERY_DESCRIBE,
    META_QUERY_FUNCTIONS, META_QUERY_INDEXES, META_QUERY_PRIMARY_KEYS, META_QUERY_ROLES,
    META_QUERY_SCHEMAS, META_QUERY_TABLES, META_QUERY_VIEWS,
};
use super::state::{DbStatus, Focus, Mode, PanelDirection, SearchTarget, SidebarSection};
use super::tls::{make_rustls_connect_insecure, make_rustls_connect_verified};
use crate::config::{
...
async fn fetch_primary_keys(client: &SharedClient, table: &str) -> Vec<String> {
    let query = META_QUERY_PRIMARY_KEYS.replace("$1", &escape_sql_string_literal(table));
...
async fn fetch_column_types(
    ...
    let query = META_QUERY_COLUMN_TYPES.replace("$1", &escape_sql_string_literal(table));
    let guard = client.lock().await;
...
impl App {
    ...
    fn execute_meta_query(&mut self, query_template: &str, table_arg: Option<&str>) {
        ...
        let query = if let Some(table) = table_arg {
            query_template.replace("$1", &escape_sql_string_literal(table))
        } else {
